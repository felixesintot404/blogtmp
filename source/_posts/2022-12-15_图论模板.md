...
{% note flat info %}
不过是魔改了有向图的tarjan，A了[P8436](https://www.luogu.com.cn/problem/P8436)而已。
{% endnote %}
```c++
...
void tarjan(int dc)
{
	...
	else if(!edge[j].bml&&insta[edge[j].to])low[dc]=min(low[dc],dfn[edge[j].to]);
	if(low[dc]==dfn[dc])
	{
		ans++;
		while(insta[dc])
		{
			nl[ans].emplace_back(sta[tot]);
			insta[sta[tot]]=false;
			tot--;
		}
	}
